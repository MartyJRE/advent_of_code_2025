...
fn main() -> Result<()> {
    ...
    fn part2<R: BufRead>(reader: R) -> Result<i64> {
        ...
        // For each row y in the rectangle, find the min/max x that are inside the polygon
        // and verify the rectangle's x range fits within
        // Get polygon x-range at a given y
        fn get_poly_x_range(y: i64, vs: &Vec<(i64, i64)>) -> Option<(i64, i64)> {
            let count = vs.len();
        ...
        fn is_rect_inside(
            ...
            // Only check at critical y-values: rectangle bounds + polygon vertices within rect
